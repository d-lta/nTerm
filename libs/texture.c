 ...
 */
static void *screen_buffer = NULL;

// Add this function to solve the "undefined reference" linker error
void nsp_texture_init(void) {
	if (!has_colors) {
		screen_buffer = gc_alloc(320 * 240 * 2, false);
	}
}

void nsp_texture_deinit(void) {
	// This function was originally a mess, with no body and duplicate declarations.
	// It's now correctly defined as an empty function.
}
...
static void nsp_texture_print(void (*print)(void *env, const char *fmt, ...), void *env, mp_obj_t self_in, mp_print_kind_t kind) {
	...
	nsp_texture_obj_t *self = (nsp_texture_obj_t*)self_in;
...
static mp_obj_t nsp_texture_display(mp_obj_t self_in) {
	...
	if (self->width != 320 || self->height != 240 || self->has_transparency) {
		...
		// Use lcd_blit for color calculators
		lcd_blit(self->bitmap, SCR_320x240_565);
	} else {
		// For non-color calculators, use our screen buffer
		if (!screen_buffer) {
			nlr_raise(mp_obj_new_exception_msg(&mp_type_RuntimeError, "Screen buffer not initialized!"));
			return mp_const_none;
		}

		// Flip everything, as 0xFFFF is white on CX, but black on classic
		uint16_t *ptr = self->bitmap + 320*240, *ptr_inv = ((uint16_t*)screen_buffer) + 320*240;
		uint32_t *ptr32 = (uint32_t*)ptr, *ptr_inv32 = (uint32_t*)ptr_inv;
		while (--ptr32 >= (uint32_t*)self->bitmap) {
			*--ptr_inv32 = ~*ptr32;
		}

		// Use lcd_blit instead of direct screen memory access
		lcd_blit(screen_buffer, SCR_320x240_565);
	}
...
static mp_obj_t nsp_texture_setPx(uint n_args, const mp_obj_t *args) {
	...
	nsp_texture_obj_t *self = (nsp_texture_obj_t*)args[0];
...
static mp_obj_t nsp_texture_drawOnto(uint n_args, const mp_obj_t *args, mp_map_t *kw_args) {
	...
	mp_arg_val_t args_val[MP_ARRAY_SIZE(nsp_drawOnto_args)];
	mp_arg_parse_all(n_args, args, kw_args, MP_ARRAY_SIZE(nsp_drawOnto_args), nsp_drawOnto_args, args_val);

	src = (nsp_texture_obj_t*)args_val[0].u_obj;
	dest = (nsp_texture_obj_t*)args_val[1].u_obj;

	if (mp_obj_get_type(src) != &nsp_texture_type || mp_obj_get_type(dest) != &nsp_texture_type) {
		nlr_raise(mp_obj_new_exception_msg(&mp_type_ValueError, "Wrong type of argument."));
		return mp_const_none;
	}

	if (args_val[2].u_int != (uint)-1) {
		src_x = args_val[2].u_int;
	}

	if (args_val[3].u_int != (uint)-1) {
		src_y = args_val[3].u_int;
	}

	if (args_val[4].u_int == (uint)-1) {
		src_w = src->width;
	} else {
		src_w = args_val[4].u_int;
	}

	if (args_val[5].u_int == (uint)-1) {
		src_h = src->height;
	} else {
		src_h = args_val[5].u_int;
	}

	if (args_val[6].u_int != (uint)-1) {
		dest_x = args_val[6].u_int;
	}

	if (args_val[7].u_int != (uint)-1) {
		dest_y = args_val[7].u_int;
	}

	if (args_val[8].u_int == (uint)-1) {
		dest_w = src_w;
	} else {
		dest_w = args_val[8].u_int;
	}

	if (args_val[9].u_int == (uint)-1) {
		dest_h = src_h;
	} else {
